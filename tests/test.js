...
test("return true/failed rule", () => {
  ...
  expect(Iodine.is(5, ["required", "integer", "min:7", "max:10"])).toBe(
    "min:7"
  );
  expect(Iodine.is(5, ["optional", "integer", "min:7", "max:10"])).toBe(
    "min:7"
  );
  expect(Iodine.is("", ["optional", "integer", "min:7", "max:10"])).toBe(true);
  expect(Iodine.is(null, ["optional", "integer", "min:7", "max:10"])).toBe(
    true
  );
  expect(Iodine.is(undefined, ["optional", "integer", "min:7", "max:10"])).toBe(
    true
  );
});
...
test("return true/false", () => {
  expect(Iodine.isValid("5", ["required", "string", "min:1", "max:5"])).toBe(
    true
  );
  expect(Iodine.isValid(5, ["required", "integer", "min:7", "max:10"])).toBe(
    false
  );
  expect(Iodine.isValid(5, ["optional", "integer", "min:7", "max:10"])).toBe(
    false
  );
  expect(Iodine.isValid("", ["optional", "integer", "min:7", "max:10"])).toBe(
    true
  );
  expect(Iodine.isValid(null, ["optional", "integer", "min:7", "max:10"])).toBe(
    true
  );
  expect(
    Iodine.isValid(undefined, ["optional", "integer", "min:7", "max:10"])
  ).toBe(true);
});
...
test("it can add async custom rules", async () => {
  Iodine.addRule(
    "timeoutEquals",
    (value, param) =>
      new Promise((resolve) => setTimeout(resolve(value == param), 10))
  );

  Iodine.setErrorMessages({
    timeoutEquals: `Value must be equal to '[PARAM]' after 10ms`,
  });
  ...
  expect(await Iodine.asyncIs(1, ["required", "timeoutEquals:2"])).toBe(
    "timeoutEquals:2"
  );
  expect(
